...
const repoText = String(pkg.repository || "");
const githubMatch = repoText.match(/github:(.+)\/(.+)$/);
const githubOwner = githubMatch?.[1]?.trim() || "";
const githubRepo = githubMatch?.[2]?.trim() || "";
const enableAutoUpdate = githubOwner === "imsyy" && githubRepo === "SPlayer";
...
export const checkUpdate = (win: BrowserWindow, showTip: boolean = false) => {
  if (!enableAutoUpdate) {
    updateLog.info("ℹ️ Auto update disabled for custom build");
    if (showTip) {
