...
import { useCloudSyncStore, useDataStore, useSettingStore } from "@/stores";
...
export const openUserAgreement = async (popup?: CloudPopup | null) => {
  ...
  const popupData = popup && cloudSyncStore.shouldShowPopup(popup) ? popup : null;
  // 如果已经同意了当前版本，则不需要再弹窗
  if (!needReAgree && !popupData) return;
  ...
  const modal = window.$modal.create({
    ...
    onEsc: () => {
      if (needReAgree) window.$message.warning("请先阅读并同意用户协议");
    },
  });
};
...
// 云端同步
export const openCloudSync = async () => {
  if (isModalOpen("cloud-sync", "云端面板已打开")) return;
  setModalOpen("cloud-sync");
  const { default: CloudSync } = await import("@/components/Modal/CloudSync.vue");
  const modal = window.$modal.create({
    preset: "card",
    transformOrigin: "center",
    autoFocus: false,
...
// 添加到歌单
export const openPlaylistAdd = async (data: SongType[], mode: PlaylistStorageMode = "online") => {
  if (!data.length) return window.$message.warning("请正确选择歌曲");
  if (!isLogin() && mode === "online") return openUserLogin();
  ...
  const modal = window.$modal.create({
    ...
    autoFocus: false,
    style: { width: "600px" },
    title: mode === "local" ? "添加到本地歌单" : mode === "my" ? "添加到我的歌单" : "添加到歌单",
    content: () => {
      return h(PlaylistAdd, { data, mode, onClose: () => modal.destroy() });
    },
  });
};
...
// 新建歌单
export const openCreatePlaylist = async (mode: PlaylistStorageMode = "online") => {
  ...
  const modal = window.$modal.create({
    ...
    autoFocus: false,
    style: { width: "600px" },
    title: mode === "local" ? "新建本地歌单" : mode === "my" ? "新建我的歌单" : "新建歌单",
    content: () => {
      return h(CreatePlaylist, { mode, onClose: () => modal.destroy() });
    },
  });
};
...
export const openUpdatePlaylist = async (
  ...
  data: CoverType,
  func: () => Promise<void>,
  mode: PlaylistStorageMode = "online",
) => {
  ...
  const modal = window.$modal.create({
    ...
    autoFocus: false,
    style: { width: "600px" },
    title: mode === "local" ? "编辑本地歌单" : mode === "my" ? "编辑我的歌单" : "编辑歌单",
    content: () => {
