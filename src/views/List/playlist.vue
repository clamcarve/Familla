...
<template>
  <div class="playlist-list">
    <ListDetail
      :detail-data="isCurrentPlaylistDetail ? detailData : null"
      :list-data="isCurrentPlaylistDetail ? listData : []"
      :loading="showLoading"
      ...
      :hide-comment-tab="
        isLocalPlaylist || isMyPlaylist || isThirdPartyPlaylist || detailData?.privacy === 10
      "
    ...
    >
      <template #action-buttons>
        ...
        <n-button
          v-else-if="!isThirdPartyPlaylist && !isMyPlaylist"
          :focusable="false"
          strong
          secondary
          round
    ...
    <template v-if="currentTab === 'songs'">
      <SongList
        ...
        :data="isCurrentPlaylistDetail ? displayData : []"
        :loading="loading"
...
import { getPlatformCover, getPlatformPlaylist, getPlatformToplist } from "@/api/tunefree";
...
import { useDataStore, useLocalStore, useMyPlaylistStore, useStatusStore } from "@/stores";
...
const playlistQueryId = computed(() => String(router.currentRoute.value.query.id || "").trim());
const playlistPlatform = computed(() =>
  String(router.currentRoute.value.query.platform || "")
    .trim()
    .toLowerCase(),
);
const isThirdPartyPlaylist = computed(() => router.currentRoute.value.query.thirdParty === "1");
const isMyPlaylist = computed(() => router.currentRoute.value.query.myPlaylist === "1");
...
const isUserPlaylist = computed(() => {
  if (isLocalPlaylist.value || isMyPlaylist.value) return true;
...
const isCurrentPlaylistDetail = computed(
  () => String(detailData.value?.id || "").trim() === playlistQueryId.value,
);
...
const listConfig = computed(() => ({
  ...
  showCoverMask: true,
  showPlayCount: !isLocalPlaylist.value && !isMyPlaylist.value && !isThirdPartyPlaylist.value,
  showArtist: false,
  showCreator: !isLocalPlaylist.value && !isMyPlaylist.value && !isThirdPartyPlaylist.value,
  showCount: false,
...
const fillKuwoSongCovers = async (songs: SongType[]) => {
  if (playlistPlatform.value !== "kuwo" || songs.length === 0) return songs;
...
const moreOptions = computed<DropdownOption[]>(() => [
  {
    label: "刷新缓存",
    key: "refresh",
    show: !isLocalPlaylist.value && !isMyPlaylist.value && !isThirdPartyPlaylist.value,
    props: {
  ...
  },
  {
    ...
    show:
      ...
      !isMyPlaylist.value &&
      !isThirdPartyPlaylist.value &&
      detailData.value?.privacy === 10,
...
const getPlaylistDetail = async (
  ...
  if (id.toString().length === 16 && !myPlaylistStore.isInitialized) {
    try {
      await myPlaylistStore.readMyPlaylists();
    } catch (e) {
      window.$message.error("获取我的歌单失败");
      console.error("Failed to init my playlists", e);
    }
  }
  // 判断是否为本地歌单，本地歌单 ID 为 16 位
  const isLocal = localStore.isLocalPlaylist(id);
  const isMy = myPlaylistStore.isMyPlaylist(id);
  // 本地歌单
  if (isLocal) handleLocalPlaylist(id);
  else if (isMy || isMyPlaylist.value) handleMyPlaylist(id);
  else if (isThirdPartyPlaylist.value && playlistPlatform.value) {
    await handleThirdPartyPlaylist(String(id));
  }
  // 在线歌单
  else {
    try {
...
const handleMyPlaylist = (id: number) => {
  const playlist = myPlaylistStore.getMyPlaylistDetail(id);
  if (!playlist) {
    window.$message.error("我的歌单不存在");
    setLoading(false);
    return;
  }
  const cover = playlist.cover || playlist.songs[0]?.cover || "/images/album.jpg?asset";
  setDetailData({
    id: playlist.id,
    name: playlist.name,
    cover,
  ...
  setListData(playlist.songs);
  setLoading(false);
};

const handleThirdPartyPlaylist = async (id: string) => {
  const platform = playlistPlatform.value as "qq" | "kuwo" | "kugou" | "netease";
  let result:
    | {
        detail: typeof detailData.value;
        songs: SongType[];
      }
    | undefined;
  try {
    result = await getPlatformToplist(platform, id).catch(async () =>
      getPlatformPlaylist(platform, id),
    );
  } catch (error) {
    console.error("Failed to fetch third party playlist detail", error);
    setLoading(false);
    window.$message.error("获取第三方歌单失败");
    return;
  }
  if (!result?.detail) {
    setLoading(false);
  ...
  setListData(await fillKuwoSongCovers(result.songs));
  setLoading(false);
};
...
const toDeletePlaylist = async () => {
  ...
  window.$dialog.warning({
    ...
    onPositiveClick: async () => {
      ...
      if (isMyPlaylist.value) {
        const success = await myPlaylistStore.deleteMyPlaylist(playlistId.value);
        if (success) {
          window.$message.success("我的歌单删除成功");
          router.back();
        } else {
          window.$message.error("删除失败");
        }
...
const removeSong = async (ids: number[]) => {
  ...
  if (isMyPlaylist.value) {
    const songKeys = ids
      .map((id) => listData.value.find((song) => song.id === id))
      .filter((song): song is SongType => !!song)
      .map((song) => myPlaylistStore.buildSongKey(song));
    const success = await myPlaylistStore.removeSongsFromMyPlaylist(playlistId.value, songKeys);
    if (!success) {
      window.$message.error("删除失败");
      return;
    }
...
const handleReorder = async (fromIndex: number, toIndex: number) => {
  ...
  if (isLocalPlaylist.value) {
  ...
  } else if (isMyPlaylist.value) {
    const success = await myPlaylistStore.reorderSongsInMyPlaylist(
      playlistId.value,
      fromIndex,
      toIndex,
    );
    if (!success) {
      window.$message.error("排序失败");
      handleMyPlaylist(playlistId.value);
    }
  } else {
    // 在线歌单持久化
    try {
...
const updatePlaylist = () => {
  ...
  openUpdatePlaylist(
    ...
    isLocalPlaylist.value ? "local" : isMyPlaylist.value ? "my" : "online",
  );
};
...
onBeforeRouteUpdate((to) => {
  const rawId = String(to.query.id || "").trim();
  const id = Number(rawId);
  if (rawId) {
    currentTab.value = "songs";
    oldPlaylistId.value = Number.isNaN(id) ? 0 : id;
    getPlaylistDetail(Number.isNaN(id) ? 0 : id);
  }
});
