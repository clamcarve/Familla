<template>
  <div class="search">
    ...
    <n-tabs v-model:value="searchType" class="tabs" type="segment" @update:value="tabChange">
      ...
      <n-tab v-if="!isThirdParty" name="search-playlists"> 歌单 </n-tab>
      <n-tab v-if="!isThirdParty" name="search-artists"> 歌手 </n-tab>
      <n-tab v-if="!isThirdParty" name="search-albums"> 专辑 </n-tab>
      <n-tab v-if="!isThirdParty" name="search-videos"> 视频 </n-tab>
      <n-tab v-if="!isThirdParty" name="search-radios"> 播客 </n-tab>
    </n-tabs>
    <!-- 路由 -->
...
const isThirdParty = computed(() => platformStore.currentPlatform !== "netease");
...
watch(
  ...
  (name) => {
    if (name && name.toString().startsWith("search-")) {
      searchType.value = isThirdParty.value ? "search-songs" : (name as string);
    }
  },
  { immediate: true },
);
...
watch(
  ...
  (platform) => {
    if (platform !== "netease" && route.name !== "search-songs") {
      router.replace({
        name: "search-songs",
        query: {
          ...route.query,
