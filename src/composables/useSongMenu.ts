...
export const useSongMenu = () => {
  ...
  const resolvePlaylistMode = (song: SongType): "online" | "local" | "my" => {
    if (song.path) return "local";
    if ((song.platform || platformStore.currentPlatform) !== "netease") return "my";
    return "online";
  };

  const isThirdPartySong = (song: SongType) => {
    return !!song.platform && song.platform !== "netease";
  };
  ...
  const getMenuOptions = (
    ...
    const isThirdParty = isThirdPartySong(song);
    const playlistMode = resolvePlaylistMode(song);
    ...
    const isMyPlaylist = myPlaylistStore.isMyPlaylist(playListId);
    ...
    return [
      ...
      },
      {
        ...
        props: {
          onClick: () => openPlaylistAdd([song], playlistMode),
        },
      ...
      },
      {
        key: "comment",
        label: "查看评论",
        show: !isLocal && !isThirdParty && type !== "streaming",
        props: {
          onClick: () => {
      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            key: "copy-song-info",
            label: "复制更多信息",
            show: !isLocal && !isThirdParty && type === "song",
            props: {
      ...
      },
      {
        ...
        show:
          ...
          emit !== undefined &&
          isUserPlaylist &&
          (isLocalPlaylist || isMyPlaylist || isLoginNormal) &&
          !isCloud,
        props: {
          onClick: () =>
            isMyPlaylist
      ...
      },
      {
        key: "wiki",
        label: "音乐百科",
        show: settingStore.contextMenuOptions.wiki && type === "song" && !isLocal && !isThirdParty,
        props: {
