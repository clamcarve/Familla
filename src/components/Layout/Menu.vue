...
import { openCloudSync, openCreatePlaylist } from "@/utils/modal";
...
const myPlaylistMenu = computed<MenuOption[]>(() => {
  const playlists = myPlaylistStore.myPlaylists;
  if (!playlists || playlists.length === 0) return [];
  return playlists.map((playlist) => ({
    key: `my-${playlist.id}`,
    label: () =>
...
const menuUpdate = (key: string, item: MenuOption) => {
  ...
  } else if (typeof key === "string" && key.startsWith("my-")) {
    const myId = key.replace("my-", "");
    const isMyPlaylist = myPlaylistStore.isMyPlaylist(myId);
    if (isMyPlaylist) {
      router.push({
        name: "playlist",
        query: { id: myId, myPlaylist: "1" },
      });
    }
  } else {
    switch (key) {
  ...
  switch (routerName) {
    case "playlist": {
      ...
      if (isUserPlaylist) {
        ...
        menuActiveKey.value = `my-${playlistId}`;
        menuRef.value?.showOption(`my-${playlistId}`);
      } else {
