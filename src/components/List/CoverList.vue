...
import { getPlatformPlaylist, getPlatformToplist } from "@/api/tunefree";
...
// 是否为第三方歌单
const isThirdPartyPlaylistItem = (item: CoverType) => {
  return props.type === "playlist" && !!item.platform && item.platform !== "netease";
};

// 构建详情页参数
const getDetailQuery = (item: CoverType) => {
  if (isThirdPartyPlaylistItem(item)) {
    return {
      id: item.id,
...
const getListData = async (id: number | string): Promise<SongType[]> => {
  ...
  switch (props.type) {
    ...
    case "playlist": {
      const thirdPartyItem = props.data.find((item) => String(item.id) === String(id));
      if (thirdPartyItem && isThirdPartyPlaylistItem(thirdPartyItem)) {
        const platform = thirdPartyItem.platform!;
        const isToplist = thirdPartyItem.sourceType === "toplist";
        const result = isToplist
          ? await getPlatformToplist(platform, String(id))
          : await getPlatformPlaylist(platform, String(id));
        return result.songs;
      }
      // 本地歌单
      if (isLocalPlaylist) {
