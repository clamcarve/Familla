...
import { usePlatformStore, useStatusStore, useDataStore, useSettingStore } from "@/stores";
...
const updatePlaceholder = async () => {
  if (!settingStore.enableSearchKeyword) {
    searchPlaceholder.value =
      platformStore.currentPlatform === "netease"
        ? "搜索音乐 / 视频"
        : `搜索${platformStore.currentPlatformName}歌曲`;
    return;
  }
  if (platformStore.currentPlatform !== "netease") {
    searchPlaceholder.value = `搜索${platformStore.currentPlatformName}歌曲`;
    return;
  }
  try {
...
const toSearch = async (key: any, type: string = "keyword") => {
  ...
  switch (type) {
    case "keyword":
      router.push({
        name: "search-songs",
        query: { keyword: key, platform: platformStore.currentPlatform },
      });
