<template>
  <div class="create-playlist">
    <!-- 在线歌单表单 -->
    <template v-if="props.mode === 'online'">
...
import type { PlaylistStorageMode } from "@/types/main";
...
const toCreatePlaylist = debounce(
  async (e: MouseEvent) => {
    ...
    if (props.mode === "online") {
      ...
      try {
        await localFormRef.value?.validate();
        await myPlaylistStore.createMyPlaylist(
          localFormData.value.name,
          localFormData.value.description,
        );
        emit("close");
        window.$message.success("新建我的歌单成功");
      } catch (error) {
        if (error) {
          return;
        }
        window.$message.error("新建我的歌单失败，请重试");
      }
    }
  },
  300,
