<template>
  <div class="update-playlist">
    <n-form ref="updateFormRef" :model="updateFormData" :rules="updateFormRules">
      ...
      <n-form-item v-if="props.mode === 'online'" label="歌单分类" path="tags">
        <n-select
...
import type { CoverType, PlaylistStorageMode } from "@/types/main";
...
const isMy = computed(() => props.mode === "my");
...
const toUpdatePlaylist = debounce(
  async (e: MouseEvent) => {
    ...
    if (isMy.value) {
      const success = await myPlaylistStore.updateMyPlaylist(props.id, {
        name: updateFormData.value.name,
        description: updateFormData.value.desc,
      });
      if (success) {
        emit("success");
        window.$message.success("我的歌单编辑成功");
      } else {
        window.$message.error("我的歌单编辑失败");
      }
      return;
    }
