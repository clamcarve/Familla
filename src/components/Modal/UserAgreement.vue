...
<template>
  <div class="user-agreement">
    <n-h1 class="title">{{ panelTitle }}</n-h1>
    <n-scrollbar class="scrollbar">
      <div v-if="popup" class="popup-section">
        <n-flex class="date" justify="center">
          <n-tag round>公告模块</n-tag>
          <n-tag type="info" round>更新时间：{{ popupUpdatedAt }}</n-tag>
        </n-flex>
        <n-card>
          <n-flex vertical :size="12">
            <n-h3 prefix="bar">{{ popup.title || "软件公告" }}</n-h3>
            <div class="markdown-body" v-html="popupHtml" @click="handleMarkdownClick" />
          </n-flex>
        </n-card>
      </div>
      <template v-if="showAgreement">
        ...
        <n-alert type="warning" title="重要声明">
          Familla 是一款完全免费且开源的音乐播放软件，遵循 AGPL-3.0 开源协议发布。
          本软件永远不会向用户收取任何费用。如果您是从第三方渠道付费购买本软件，
          您可能遇到了诈骗，请立即停止使用并举报。
        </n-alert>
        <n-p>
          欢迎使用 Familla（以下简称"本软件"）。本软件是一个本地音乐播放软件，可能会调用第三方 API
          来增强其功能。请在使用本软件前仔细阅读本用户协议。本协议是您（以下简称"用户"）与 Familla
          开发团队（以下简称"开发者"）之间的法律协议。一旦您下载、安装或使用本软件，即表示您同意接受本协议的所有条款和条件。
        </n-p>
        <n-h3 prefix="bar"> 软件使用 </n-h3>
        <n-ol>
          <n-li>本软件基于开源软件SPlayer修改：https://github.com/imsyy/SPlayer</n-li>
        ...
        <n-ol>
          ...
          <n-li>用户在使用、修改和分发 Familla 时，必须遵守 AGPL-3.0 许可的条款和条件。</n-li>
          <n-li>
        ...
        <n-card ref="readOverRef">
          若你（使用者）接受以上协议，请点击下面的“同意并继续”按钮签署本协议，若不接受，请点击“不同意”后软件将退出并清除本软件的所有数据。
        </n-card>
      </template>
    </n-scrollbar>
    <n-flex justify="center">
      <n-button v-if="showAgreement && isElectron" type="error" @click="closeApp">不同意</n-button>
      <n-button type="success" @click="confirmAction" :disabled="showAgreement && !isReadOver">
        {{ confirmButtonText }}
      </n-button>
    </n-flex>
  </div>
...
import type { CloudPopup } from "@/api/cloudSync";
import { marked } from "marked";
import { isElectron } from "@/utils/env";
import { useCloudSyncStore, useSettingStore } from "@/stores";
import { CURRENT_AGREEMENT_VERSION } from "@/constants/agreement";

const props = withDefaults(
  defineProps<{
    popup?: CloudPopup | null;
...
const panelTitle = computed(() => {
  if (props.showAgreement) return "软件许可使用协议";
  return props.popup?.title || "软件公告";
});

const confirmButtonText = computed(() => {
  if (props.showAgreement) {
    return isReadOver.value ? "同意并继续" : "请先完整阅读";
  }
  return "我知道了";
});

const popupUpdatedAt = computed(() => {
  if (!props.popup?.updated_at) return "-";
  return new Date(props.popup.updated_at).toLocaleString("zh-CN", {
    hour12: false,
  });
});

const markPopupRead = () => {
  if (props.popup) cloudSyncStore.markPopupRead(props.popup);
};

const confirmAction = () => {
  if (props.showAgreement && !isReadOver.value) return;
  if (props.showAgreement) {
...
watchEffect(async () => {
  popupHtml.value = props.popup?.content_md ? await marked(props.popup.content_md) : "";
});
</script>
