    ...
    <n-flex class="batch-footer" justify="space-between" align="center">
      ...
      <n-flex class="menu">
        ...
        <n-button
          ...
          strong
          secondary
          @click="handleDeleteSongsFromPlaylist"
        >
          <template #icon>
            <SvgIcon name="Delete" />
          </template>
        ...
        <n-button
          ...
          @click="
            openPlaylistAdd(
              ...
              props.isLocal
                ? 'local'
                : checkSongData.some((item) => item.platform && item.platform !== 'netease')
                  ? 'my'
                  : 'online',
            )
          "
...
import { useLocalStore, useMyPlaylistStore, useStatusStore } from "@/stores";
...
const isMyPlaylist = computed(() => myPlaylistStore.isMyPlaylist(props.playListId));
...
const handleDeleteSongsFromPlaylist = async () => {
  if (!props.playListId || checkSongData.value.length === 0) return;
  if (!isMyPlaylist.value) {
    deleteSongs(
      props.playListId,
      checkSongData.value.map((item) => item.id),
      {
        songName: checkSongData.value.length === 1 ? checkSongData.value[0].name : undefined,
      },
    );
    return;
  }
  window.$dialog.warning({
    ...
    content:
      checkSongData.value.length > 1
        ? "确定删除这些选中的歌曲吗？"
        : checkSongData.value[0]?.name
          ? `确定删除歌曲 ${checkSongData.value[0].name} 吗？`
          : "确定删除这个歌曲吗？",
    positiveText: "删除",
    negativeText: "取消",
    onPositiveClick: async () => {
      const songKeys = checkSongData.value.map((song) => myPlaylistStore.buildSongKey(song));
      const success = await myPlaylistStore.removeSongsFromMyPlaylist(props.playListId!, songKeys);
      if (success) {
