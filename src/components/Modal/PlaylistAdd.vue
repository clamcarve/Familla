  ...
  <div class="playlist-add">
    ...
    <template v-if="mode === 'online'">
      <n-scrollbar style="max-height: 70vh">
        <n-list class="playlists-list" hoverable clickable>
          <!-- 新建歌单 -->
          <n-list-item class="playlist add" @click="openCreatePlaylist('online')">
            <template #prefix>
    ...
    <template v-else-if="mode === 'local'">
      <n-scrollbar style="max-height: 70vh">
        <n-list class="playlists-list" hoverable clickable>
          <!-- 新建本地歌单 -->
          <n-list-item class="playlist add" @click="openCreatePlaylist('local')">
            <template #prefix>
              <SvgIcon name="Add" :size="20" />
            </template>
            <n-thing title="创建新歌单" />
    ...
    <template v-else>
      <n-scrollbar style="max-height: 70vh">
        <n-list class="playlists-list" hoverable clickable>
          <n-list-item class="playlist add" @click="openCreatePlaylist('my')">
            <template #prefix>
              <SvgIcon name="Add" :size="20" />
            </template>
            <n-thing title="创建我的歌单" />
          </n-list-item>
          <template v-if="myPlaylists.length > 0">
            <n-list-item
          ...
          <n-empty v-else description="暂无我的歌单" style="padding: 40px 0" />
        </n-list>
      </n-scrollbar>
    </template>
  </div>
...
import type { PlaylistStorageMode, SongType } from "@/types/main";
import type { MessageReactive } from "naive-ui";
import { useDataStore, useLocalStore, useMyPlaylistStore } from "@/stores";
...
// 我的歌单
const myPlaylists = computed(() => myPlaylistStore.myPlaylists);
...
// 添加到我的歌单
const addToMyPlaylist = debounce(
  async (playlistId: number) => {
    loadingMsg.value = window.$message.loading("正在添加歌曲至我的歌单", { duration: 0 });
    try {
      const result = await myPlaylistStore.addSongsToMyPlaylist(playlistId, props.data);
      if (loadingMsg.value) loadingMsg.value.destroy();
      if (result.success) {
        emit("close");
        if (result.addedCount > 0) {
          window.$message.success(`成功添加 ${result.addedCount} 首歌曲至我的歌单`);
        } else {
          window.$message.info("所选歌曲已在歌单中");
        }
      } else {
