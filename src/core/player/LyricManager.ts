...
import { getPlatformLyric } from "@/api/tunefree";
...
class LyricManager {
   ...
   * @returns 歌词数据和元数据
   */
  private async fetchThirdPartyLyric(song: SongType): Promise<LyricFetchResult> {
    const result: SongLyric = { lrcData: [], yrcData: [] };
    ...
    try {
      const response = await getPlatformLyric(platform, song);
      if (!response) return { data: result, meta: defaultMeta };

      if (response.qrc) {
        const qrcLines = parseQRCLyric(response.qrc, response.trans, response.roma);
        if (qrcLines.length > 0) {
          result.yrcData = qrcLines;
          return {
            data: result,
            meta: { usingTTMLLyric: false, usingQRCLyric: true },
          };
        }
      }

      let lrcLines = response.lrc ? parseLrc(response.lrc) : [];
      if (response.trans) {
        const transLines = parseLrc(response.trans);
        if (transLines.length > 0) {
          lrcLines = alignLyrics(lrcLines, transLines, "translatedLyric");
        }
      }
      if (response.roma) {
        const romaLines = parseLrc(response.roma);
  ...
  public async fetchLyric(song: SongType): Promise<LyricFetchResult> {
    ...
    try {
      ...
      if (isStreaming) {
        ...
        } else if (song.platform && song.platform !== "netease") {
          // 第三方平台
          fetchResult = await this.fetchThirdPartyLyric(song);
        } else {
          // 在线获取
