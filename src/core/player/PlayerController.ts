...
import { getPlatformCover } from "@/api/tunefree";
...
class PlayerController {
   ...
   * @param song 当前歌曲
   */
  private async updateThirdPartyCover(song: SongType) {
    if (!song.platform || song.platform === "netease") return;

    const musicStore = useMusicStore();
    const currentTrackId =
      musicStore.playSong.platformSongId || String(musicStore.playSong.id || "").trim();
    const targetTrackId = song.platformSongId || String(song.id || "").trim();
    if (
      currentTrackId === targetTrackId &&
      musicStore.playSong.cover &&
      !musicStore.playSong.cover.includes("/images/song.jpg?asset")
    ) {
      return;
    }

    try {
      const cover = await getPlatformCover(song.platform, song);
      if (!cover) return;

      const latestTrackId =
        musicStore.playSong.platformSongId || String(musicStore.playSong.id || "").trim();
      if (latestTrackId !== targetTrackId) return;

      musicStore.playSong.cover = cover;
      musicStore.playSong.coverSize = {
        s: cover,
        m: cover,
        l: cover,
        xl: cover,
  ...
  public async afterPlaySetup(song: SongType) {
    ...
    if (!song.path || song.type === "streaming") {
      ...
      if (song.platform && song.platform !== "netease") {
        this.updateThirdPartyCover(song);
      }
    }
    // 本地文件额外处理
    else {
  ...
  private bindAudioEvents() {
    ...
    audioManager.addEventListener("play", () => {
      ...
      window.document.title = `${playTitle} | Familla`;
      // 只有真正播放了才重置重试计数
    ...
    audioManager.addEventListener("pause", () => {
      ...
      if (!isElectron) window.document.title = "Familla";
      playerIpc.sendPlayStatus(false);
      playerIpc.sendTaskbarState({ isPlaying: false });
      playerIpc.sendTaskbarMode("paused");
      playerIpc.sendTaskbarProgress(statusStore.progress);
